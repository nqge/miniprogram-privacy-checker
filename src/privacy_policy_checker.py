...
class PrivacyPolicyChecker:
    ...
    def _find_privacy_policy_files(self):
        ...
        if self.privacy_policy_files:
            ...
            # 尝试使用项目根目录的隐私协议.txt文件
            protocol_file = self.miniprogram_path / '隐私协议.txt'
            if protocol_file.exists():
                print(f"[+] 使用项目根目录的隐私协议.txt文件: {protocol_file}")
                self.privacy_policy_files.append(protocol_file)
            else:
                print("[-] 未找到隐私政策文件")
                self.issues.append({
                    'type': 'critical',
                    'message': '未找到隐私政策文件',
                    'suggestion': '请在小程序根目录或 pages 目录下创建隐私政策文件（如 privacy.md、隐私政策.txt），或确保项目根目录存在 隐私协议.txt 文件'
                })
